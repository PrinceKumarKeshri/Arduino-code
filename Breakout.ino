...
void setup(void) {
  ...
  Serial.print(F("Loading 2.bmp to screen..."));
  stat = reader.drawBMP("/2.bmp", tft, 0, 0);
  reader.printStatus(stat);   // How'd we do?

  // Query the dimensions of image 'miniwoof.bmp' WITHOUT loading to screen:
  Serial.print(F("Querying 1.bmp image size..."));
  stat = reader.bmpDimensions("/1.bmp", &width, &height);
  reader.printStatus(stat);   // How'd we do?
  ...
  Serial.print(F("Loading 2.bmp to canvas..."));
  stat = reader.loadBMP("/2.bmp", img);
  reader.printStatus(stat); // How'd we do?
